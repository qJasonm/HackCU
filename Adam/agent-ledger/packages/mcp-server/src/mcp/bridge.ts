...
async function ledgerRequest(
  ...
  if (requireAuth) {
    if (!currentToken) {
      throw new Error(
        'Not registered. Call ledger_register first to get a token.'
      );
    }
  ...
  switch (name) {
    ...
    case 'ledger_register': {
      const result = await ledgerRequest(
        'POST',
        '/auth/register',
        { agent_id: args.agent_id, tier: args.tier },
        false
      );
    ...
    case 'ledger_read_history': {
      ...
      const result = await ledgerRequest(
        'GET',
        `/ledger/latest?agent_id=${agentId}&count=${count}`
      );
    ...
    case 'ledger_search': {
      ...
      const result = await ledgerRequest(
        'GET',
        `/ledger/query?q=${encodeURIComponent(args.query)}&limit=${limit}`
      );

      const results = result.results.map((e: any) => ({
        event_id: e.event_id,
        agent: e.agent_id,
        time: new Date(e.created_at * 1000).toISOString(),
        resource: e.resource,
      ...
      return JSON.stringify(
        {
          ...
          count: result.count,
          pending: result.pending.map((e: any) => ({
            event_id: e.event_id,
            resource: e.resource,
