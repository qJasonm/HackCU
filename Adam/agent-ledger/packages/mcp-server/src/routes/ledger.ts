...
import type { ActiveTailService } from '../services/active-tail.js';
import type { AuditLogService } from '../services/audit-log.js';

export function ledgerRoutes(
  db: Database.Database,
  config: LedgerConfig,
  activeTail: ActiveTailService,
  auditLog: AuditLogService
): Router {
  const router = Router();

  // POST /ledger/report-intent — Log intent before action
  router.post('/report-intent', (req: Request, res: Response) => {
    try {
      const agent = req.agent!;
      const request = req.body as ReportIntentRequest;
      ...
      const result = createEvent(
        agent.agent_id,
        agent.tier as TrustTier,
        request,
        config.event_tree.max_tree_depth,
        db
      );
  ...
  router.post('/report-action', (req: Request, res: Response) => {
    try {
      const agent = req.agent!;
      const request = req.body as ReportActionRequest;
  ...
  router.get('/latest', (req: Request, res: Response) => {
    try {
      const agent = req.agent!;
      const count = parseInt(req.query.count as string) || config.storage.active_tail_size;
      const agentId = (req.query.agent_id as string) || agent.agent_id;
  ...
  router.get('/query', (req: Request, res: Response) => {
    try {
      ...
      const events = db.prepare(
        "SELECT * FROM events WHERE intent LIKE ? ORDER BY created_at DESC LIMIT ?"
      ).all(`%${query}%`, limit) as LedgerEvent[];
