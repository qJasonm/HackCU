...
import {
  insertExternalEdit,
  getUnresolvedExternalEdits,
  insertSyncAck,
  getSyncAcks,
  getActiveLeasesByResource,
  updateLeaseStatus,
  LeaseStatus,
  type Lease,
  type ExternalEditNotification,
...
export function syncRoutes(db: Database.Database): Router {
  ...
  router.post('/notify-external-edit', (req: Request, res: Response) => {
    try {
      ...
      const activeLeases = getActiveLeasesByResource(db).all(resource) as Lease[];
      const affectedAgents: string[] = [];

      for (const lease of activeLeases) {
        // Check overlap
        const overlaps = (
          lease.line_start === null || lease.line_end === null ||
  ...
  router.post('/acknowledge', (req: Request, res: Response) => {
    try {
      ...
      const acks = getSyncAcks(db).all(event_id) as any[];
      const edit = db.prepare('SELECT * FROM external_edits WHERE event_id = ?').get(event_id) as any;

      if (edit) {
        const activeLeases = getActiveLeasesByResource(db).all(edit.resource) as Lease[];
        const affectedAgentIds = [...new Set(activeLeases.map(l => l.agent_id))];
        const ackedAgentIds = new Set(acks.map((a: any) => a.agent_id));
  ...
  router.get('/pending', (req: Request, res: Response) => {
    try {
      ...
      const leases = db.prepare(
        "SELECT DISTINCT resource FROM leases WHERE agent_id = ? AND status IN ('active', 'suspended')"
      ).all(agent.agent_id) as { resource: string }[];

      const pendingEdits: any[] = [];
      for (const { resource } of leases) {
        const edits = getUnresolvedExternalEdits(db).all(resource) as any[];
        for (const edit of edits) {
          // Check if this agent already acknowledged
          const ack = db.prepare(
            'SELECT 1 FROM sync_acks WHERE event_id = ? AND agent_id = ?'
          ).get(edit.event_id, agent.agent_id);
