...
export function leaseRoutes(db: Database.Database, config: LedgerConfig, activeTail: ActiveTailService): Router {
  const router = Router();
  ...
  router.post('/acquire', (req: Request, res: Response) => {
    try {
      ...
      const request = req.body as LeaseRequest;
      ...
      const result = acquireLease(agent.agent_id, agent.tier as TrustTier, request, db);
  ...
  router.post('/escalate', (req: Request, res: Response) => {
    try {
      ...
      const result = escalateLease(lease_id, new_scope as LeaseScope, line_start ?? null, line_end ?? null, db);
