...
export enum Priority {
  ...
  P2 = 2, // Worker new lease requests
}
...
export interface LeaseRequest {
  ...
  line_start?: number;
  line_end?: number;
  symbol_name?: string;  // For symbol scope — resolved to lines by server
}
...
export interface LedgerConfig {
  jwt: {
    mode: JWTMode;
    secret_path?: string;       // HS256
    private_key_path?: string;  // RS256
    public_key_path?: string;   // RS256
    expiry_hours: number;
  };
  server: {
    port: number;
...
// ---- Sync / Unauthorized Edit ----

export interface ExternalEditNotification {
  event_id: string;
  resource: string;
  changed_lines: { start: number; end: number };
  detected_at: number;
  detected_by: string; // e.g. 'vscode-extension'
}
...
export interface LeaseAcquireResponse {
  ...
  request_id?: string;
  warning?: string;  // e.g. SYMBOL_UNRESOLVABLE
}
...
export const MAX_CONCURRENT_LEASES: Record<TrustTier, number> = {
...
export const TIER_SCOPES: Record<TrustTier, string[]> = {
