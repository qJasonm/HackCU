...
export function insertAgent(db: Database.Database): Stmt {
  return db.prepare(`
...
export function getAgentById(db: Database.Database): Stmt {
  return db.prepare('SELECT * FROM agents WHERE agent_id = ?');
}

export function getAgentBySessionId(db: Database.Database): Stmt {
  return db.prepare('SELECT * FROM agents WHERE session_id = ?');
}

export function updateAgentHeartbeat(db: Database.Database): Stmt {
  return db.prepare('UPDATE agents SET last_heartbeat = ? WHERE agent_id = ?');
}

export function insertLease(db: Database.Database): Stmt {
  return db.prepare(`
...
export function getActiveLeasesByResource(db: Database.Database): Stmt {
  return db.prepare("SELECT * FROM leases WHERE resource = ? AND status = 'active'");
}

export function getActiveLeasesByAgent(db: Database.Database): Stmt {
  return db.prepare("SELECT * FROM leases WHERE agent_id = ? AND status = 'active'");
}

export function updateLeaseStatus(db: Database.Database): Stmt {
  return db.prepare('UPDATE leases SET status = ? WHERE lease_id = ?');
}

export function updateLeaseRange(db: Database.Database): Stmt {
  return db.prepare('UPDATE leases SET line_start = ?, line_end = ? WHERE lease_id = ?');
}

export function insertEvent(db: Database.Database): Stmt {
  return db.prepare(`
...
export function getEventById(db: Database.Database): Stmt {
  return db.prepare('SELECT * FROM events WHERE event_id = ?');
}

export function getEventsByCorrelationId(db: Database.Database): Stmt {
  return db.prepare('SELECT * FROM events WHERE correlation_id = ? ORDER BY created_at ASC');
}

export function getLatestEventsByAgent(db: Database.Database): Stmt {
  return db.prepare('SELECT * FROM events WHERE agent_id = ? ORDER BY created_at DESC LIMIT ?');
}

export function updateEventStatus(db: Database.Database): Stmt {
  return db.prepare('UPDATE events SET status = ?, completed_at = ? WHERE event_id = ?');
}

export function updateEventGroundTruth(db: Database.Database): Stmt {
  return db.prepare('UPDATE events SET ground_truth = ?, status = ?, completed_at = ? WHERE event_id = ?');
}

export function insertRevokedSession(db: Database.Database): Stmt {
  return db.prepare(`
...
export function isSessionRevoked(db: Database.Database): Stmt {
  return db.prepare('SELECT 1 FROM revoked_sessions WHERE session_id = ?');
}

export function insertMilestone(db: Database.Database): Stmt {
  return db.prepare(`
...
export function insertLeaseQueueEntry(db: Database.Database): Stmt {
  return db.prepare(`
...
export function getQueuedRequestsByResource(db: Database.Database): Stmt {
  return db.prepare('SELECT * FROM lease_queue WHERE resource = ? ORDER BY priority ASC, queued_at ASC');
}

export function removeQueueEntry(db: Database.Database): Stmt {
  return db.prepare('DELETE FROM lease_queue WHERE request_id = ?');
}

export function getStaleQueueEntries(db: Database.Database): Stmt {
  return db.prepare('SELECT * FROM lease_queue WHERE queued_at < ? AND priority > 0');
}

export function promoteQueueEntry(db: Database.Database): Stmt {
  return db.prepare('UPDATE lease_queue SET priority = 0, promoted_at = ? WHERE request_id = ?');
}

export function insertExternalEdit(db: Database.Database): Stmt {
  return db.prepare(`
...
export function getUnresolvedExternalEdits(db: Database.Database): Stmt {
  return db.prepare("SELECT * FROM external_edits WHERE resolved = 0 AND resource = ?");
}

export function insertSyncAck(db: Database.Database): Stmt {
  return db.prepare(`
...
export function getSyncAcks(db: Database.Database): Stmt {
