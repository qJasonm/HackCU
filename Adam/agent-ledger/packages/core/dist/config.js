...
export function loadLedgerConfig(basePath) {
    ...
    try {
        ...
        return deepMerge(DEFAULT_LEDGER_CONFIG, parsed);
    }
    catch {
...
export function loadJanitorConfig(basePath) {
    ...
    try {
        const raw = readFileSync(configPath, 'utf-8');
        const parsed = JSON.parse(raw);
        return deepMerge(DEFAULT_JANITOR_CONFIG, parsed);
    }
    catch {
        console.warn(`Failed to parse ${configPath}, using defaults.`);
        return { ...DEFAULT_JANITOR_CONFIG };
